...
public class SmsController {
    ...
    @Operation(summary = "로그인sms 인증코드 발송", description = "입력한 휴대폰 번호로 6자리 인증코드가 발송됩니다.")
    @ApiResponse(responseCode = "200", description = "발송 성공")
    @ApiResponse(responseCode = "400", description = "잘못된 요청")
    @PostMapping("/api/login/send")
    public ResponseEntity<CommonResponse<String>> sendLoginSms(@Valid @RequestBody SmsSendRequestDTO request) {
        String toNumber = request.getPhoneNumber();
        smsSolaApiService.sendVerificationLoginSms(toNumber);
        return ResponseEntity.status(SuccessCode.SMS_SEND.getStatus())
                .body(CommonResponse.of(SuccessCode.SMS_SEND, "123456"));
    }
